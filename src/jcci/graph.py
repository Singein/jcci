...
class Graph(object):
    ...
    def create_node_category(self, class_or_xml, name, type, diff_type, diff_content, file_path, documentation, body,
                             extend_dict: dict):
        category = {
            'name': class_or_xml
        }
        ...
        node_exist = [node for node in self.nodes if
                      node['name'] == node_create['name'] and node['type'] == type and node['file_path'] == file_path]
        if node_exist:
    ...
    def draw_graph(self, canvas_width, canvas_height):
        ...
        for impacted_node in impacted_nodes:
            ...
            level_node_index = level_node_list.index(impacted_node['id']) if impacted_node[
                                                                                 'id'] in level_node_list else 1
...
if __name__ == '__main__':
    ...
    relationships = [{'source': 'A', 'target': 'B'}, {'source': 'B', 'target': 'C'}, {'source': 'C', 'target': 'D'},
                     {'source': 'B', 'target': 'D'}]
