    ...
    for hunk in patch:
        if hunk.added > 0:
            ...
            for i in range(0, len(targets)):
                if targets[i].startswith('+') and not targets[i][1:].strip().startswith(('*', '//', 'import ')) and \
                        targets[i][1:].strip():
        ...
        if hunk.removed > 0:
            ...
            for i in range(0, len(sources)):
                if sources[i].startswith('-') and not sources[i][1:].strip().startswith(('*', '//', 'import ')) and \
                        sources[i][1:].strip():
...
def get_diff_info(file_path):
    ...
    for patch in patch_set:
        if '.git' in patch.path or os.path.join('src', 'test') in patch.path or (
                not patch.path.endswith(('.java', '.xml'))):
            continue
