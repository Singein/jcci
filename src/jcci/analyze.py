...
class JCCI(object):
    def __init__(self, git_url, username, output_dir=None, project_path=None, result_html_file_name=None):
        ...
        self.result_html_file_name: str = result_html_file_name or ''
        self.project_name: str = self.git_url.split('/')[-1].split('.git')[0]
        self.temp_path: str = project_path or os.path.join(os.getcwd(), 'temp')
        if not os.path.exists(self.temp_path):
            os.mkdir(self.temp_path)

        self.output_path: str = output_dir or os.path.join(os.getcwd(), 'output')
        if not os.path.exists(self.output_path):
            os.mkdir(self.output_path)

        if not os.path.exists(os.path.join(self.output_path, self.project_name)):
            os.mkdir(os.path.join(self.output_path, self.project_name))

        self.sqlite = SqliteHelper(self.temp_path + '/' + username + '_jcci.db')
    ...
    # Step 1.1
    def _can_analyze(self, cci_file_path):
        # 已有分析结果
        ...
        # 正在分析
        wait_index = 0
        occupy_filepath = os.path.join(self.temp_path, 'Occupy.ing')
        atexit.register(self._clean_occupy, occupy_filepath)
        while os.path.exists(occupy_filepath) and wait_index < 30:
            logging.info(
                f'Analyzing by others, waiting or clean occupying file manually at: {occupy_filepath} to continue')
            time.sleep(3)
            wait_index += 1
    ...
    def _clone_dependents_project(self, dependents):
        for dependent in dependents:
            ...
            dependent_file_path = os.path.join(self.temp_path, dependent_project_name)
            if not os.path.exists(dependent_file_path):
                logging.info(f'Cloning dependent project: {dependent_git_url}')
                os.system(
                    f'git clone -b {dependent_branch} {dependent_git_url} {dependent_file_path} && cd {dependent_file_path} && git reset --hard {dependent_commit_id}')
            else:
                os.system(
                    f'cd {dependent_file_path} && git fetch --all && git checkout -b {dependent_branch} origin/{dependent_branch} && git reset --hard {dependent_commit_id}')
                os.system(
                    f'cd {dependent_file_path} && git checkout -b {dependent_branch} && git reset --hard {dependent_commit_id}')
    ...
    def _get_diff_parse_map(self, filepath, branch, commit_first, commit_second):
        ...
        diff_base = f'cd {self.output_path} && git diff {commit_second}..{commit_first} > diff_{commit_second}..{commit_first}.txt'
        os.system(diff_base)
        diff_txt = os.path.join(self.output_path, f'diff_{commit_second}..{commit_first}.txt')
        logging.info(f'Analyzing diff file, location: {diff_txt}')
        return diff_parse.get_diff_info(diff_txt)

    # Step 2
    def _get_branch_diff_parse_map(self, project_dir, branch_first, branch_second):
        logging.info('Git pull project to HEAD')
        os.system(
            f'cd {project_dir} && git fetch --all && git checkout -b {branch_second} origin/{branch_second} && git checkout {branch_second} && git pull')
        time.sleep(1)
        os.system(
            f'cd {project_dir} && git fetch --all && git checkout -b {branch_first} origin/{branch_first} && git checkout {branch_first} && git pull')
        time.sleep(1)
        logging.info(f'Git diff between {branch_first} and {branch_second}')

        diff_txt_filename = f'diff_{branch_second.replace("/", "#")}..{branch_first.replace("/", "#")}.txt'

        diff_txt_file_path = os.path.join(self.output_path, self.project_name, diff_txt_filename)
        diff_base = f'cd {project_dir} && git diff {branch_second}..{branch_first} > {diff_txt_file_path}'
        os.system(diff_base)
        diff_txt = os.path.join(self.output_path, diff_txt_file_path)
        logging.info(f'Analyzing diff file, location: {diff_txt}')
        return diff_parse.get_diff_info(diff_txt)
    ...
    def _parse_project(self, project_dir, new_commit_or_branch, old_commit_or_branch):
        ...
        diff_xml_file_path = [key for key in file_path_list if key.endswith('.xml') and any(
            key.endswith(diff_path) for diff_path in self.diff_parse_map.keys())]
    ...
    def _xml_diff_analyze(self, patch_filepath, diff_parse_obj: dict):
        xml_file_path_list = [filepath for filepath in self.xml_parse_results_new.keys() if
                              filepath.endswith(patch_filepath)]
        ...
        if xml_parse_result_new:
            ...
            self._xml_method_diff_analyze(methods, diff_parse_obj['line_num_added'],
                                          diff_parse_obj['line_content_added'], xml_parse_result_new, xml_name,
                                          xml_file_path, self.commit_or_branch_new)
        ...
        if xml_parse_result_old:
            ...
            self._xml_method_diff_analyze(methods, diff_parse_obj['line_num_removed'],
                                          diff_parse_obj['line_content_removed'], xml_parse_result_old, xml_name,
                                          xml_file_path, self.commit_or_branch_old)

    # Step 4.1.1
    def _xml_method_diff_analyze(self, methods: list, line_num_list: list, line_content_list: list, xml_parse_result,
                                 xml_name, xml_file_path, commit_or_branch):
        ...
        for line_num in line_num_list:
            ...
            for method in method_changed:
                ...
                method_node_id = self.view.create_node_category(xml_name, method.name, method.type,
                                                                constant.DIFF_TYPE_CHANGED, diff_content, xml_file_path,
                                                                '', method.content, {})
                if method.type == constant.NODE_TYPE_MAPPER_STATEMENT:
                    mapper_extend_dict['method_node_id'] = method_node_id
                    self._add_to_need_analyze_obj_list('xml', namespace, None, method.name, commit_or_branch,
                                                       mapper_extend_dict)
                    continue
                for statement in xml_parse_result.statements:
                    if statement.result_map in method_changed_name or statement.include_sql in method_changed_name:
                        statement_node_id = self.view.create_node_category(xml_name, statement.name, statement.type,
                                                                           constant.DIFF_TYPE_IMPACTED, '',
                                                                           xml_file_path, '', statement.content, {})
                        self.view.create_node_link(method_node_id, statement_node_id)
                        mapper_extend_dict['method_node_id'] = statement_node_id
                        self._add_to_need_analyze_obj_list('xml', namespace, None, statement.name, commit_or_branch,
                                                           mapper_extend_dict)
    ...
    def _java_diff_analyze(self, patch_filepath: str, diff_parse_obj: dict):
        ...
        class_db = self.sqlite.select_data(
            f'''SELECT * FROM class WHERE project_id = {self.project_id} and commit_or_branch = "{self.commit_or_branch_new}" and filepath LIKE "%{patch_filepath}"''')
        for class_db_obj in class_db:
            self._java_field_method_diff_analyze(class_db_obj, diff_parse_obj['line_num_added'],
                                                 diff_parse_obj['line_content_added'], self.commit_or_branch_new)
        # old branch or commit
        if not self.commit_or_branch_old:
            return
        class_db = self.sqlite.select_data(
            f'SELECT * FROM class WHERE project_id = {self.project_id} and commit_or_branch = "{self.commit_or_branch_old}" and filepath LIKE "%{patch_filepath}"')
        for class_db_obj in class_db:
            self._java_field_method_diff_analyze(class_db_obj, diff_parse_obj['line_num_removed'],
                                                 diff_parse_obj['line_content_removed'], self.commit_or_branch_old)

    # Step 4.2.1
    def _java_field_method_diff_analyze(self, class_db: dict, line_num_list: list, line_content_list: list,
                                        commit_or_branch: str or None):
        ...
        data_in_annotation = [annotation for annotation in json.loads(class_db['annotations']) if
                              annotation['name'] in ['Data', 'Getter', 'Setter', 'Builder', 'NoArgsConstructor',
                                                     'AllArgsConstructor']]
        for line_num in line_num_list:
            diff_content = line_content_list[line_num_list.index(line_num)]
            fields_list = self.sqlite.select_data(
                f'SELECT field_id, class_id, field_type, field_name, documentation, is_static FROM field WHERE class_id = {class_db["class_id"]} AND start_line <={line_num} AND end_line >= {line_num} order by start_line asc limit 1')
            methods_list = self.sqlite.select_data(
                f'SELECT method_id, class_id, method_name, parameters, return_type, is_api, api_path, documentation, body FROM methods WHERE class_id = {class_db["class_id"]} AND start_line <={line_num} AND end_line >= {line_num} order by start_line asc limit 1')
            class_node_id = None
            if fields_list:
                ...
                if is_not_static_fields and data_in_annotation:
                    self._add_to_need_analyze_obj_list('java', f'{class_db["package_name"]}.{class_name}', None, None,
                                                       commit_or_branch, class_db)
                    class_node_id = self.view.create_node_category(class_name, 'entity', constant.NODE_TYPE_CLASS,
                                                                   constant.DIFF_TYPE_CHANGED, '', self.output_path, '',
                                                                   '', {})
                    ...
                    for field in is_not_static_fields:
                        ...
                        field_method_name += ['get' + field_name_capitalize, 'set' + field_name_capitalize,
                                              'is' + field_name_capitalize]
                    field_method_name_str = '"' + '","'.join(field_method_name) + '"'
                    field_method_db = self.sqlite.select_data(
                        f'SELECT method_id FROM methods WHERE class_id = {class_db["class_id"]} AND method_name in ({field_method_name_str})')
                    if field_method_db:
                        self._add_to_need_analyze_obj_list('java', f'{class_db["package_name"]}.{class_name}', None,
                                                           None, commit_or_branch, class_db)
                        class_node_id = self.view.create_node_category(class_name, 'entity', constant.NODE_TYPE_CLASS,
                                                                       constant.DIFF_TYPE_CHANGED, '', self.output_path,
                                                                       '', '', {})
            for field_db in fields_list:
                node_id = self.view.create_node_category(class_name, field_db['field_name'], constant.NODE_TYPE_FIELD,
                                                         constant.DIFF_TYPE_CHANGED, diff_content, class_filepath,
                                                         field_db['documentation'], '', {})
                field_db['field_node_id'] = node_id
                self._add_to_need_analyze_obj_list('java', f'{class_db["package_name"]}.{class_name}',
                                                   field_db['field_name'], None, commit_or_branch, field_db)
                if class_node_id:
            ...
            for method_db in methods_list:
                ...
                node_id = self.view.create_node_category(class_name, method_name_param, constant.NODE_TYPE_METHOD,
                                                         constant.DIFF_TYPE_CHANGED, diff_content, class_filepath,
                                                         method_db.get('documentation'), method_db.get('body'),
                                                         node_extend_dict)
                method_db['method_node_id'] = node_id
                self._add_to_need_analyze_obj_list('java', f'{class_db["package_name"]}.{class_name}', None,
                                                   method_name_param, commit_or_branch, method_db)
    ...
    def _impacted_analyze(self, need_analyze_obj: dict):
        ...
        class_db_list = self.sqlite.select_data(
            f'SELECT class_id, filepath, commit_or_branch, is_controller, annotations, extends_class, implements '
        ...
        if file_type == 'xml':
            ...
            impacted_methods = self.sqlite.select_data(
                f'SELECT method_id, class_id, method_name, parameters, return_type, is_api, api_path, documentation, body '
            ...
            for impacted_method in impacted_methods:
                ...
                impacted_method_node_id = self.view.create_node_category(class_name, method_name_param,
                                                                         constant.NODE_TYPE_METHOD,
                                                                         constant.DIFF_TYPE_IMPACTED,
                                                                         impacted_method.get('body'), class_filepath,
                                                                         impacted_method.get('documentation'),
                ...
                self._add_to_need_analyze_obj_list('java', package_class, None,
                                                   self._get_method_param_string(impacted_method), commit_or_branch,
                                                   extend_dict)
        else:
            # analyze entity use
            ...
            if not need_analyze_obj.get('field_name') and not need_analyze_obj.get('method_param'):
                class_node_id = self.view.create_node_category(class_name, 'entity', constant.NODE_TYPE_CLASS,
                                                               constant.DIFF_TYPE_IMPACTED, '', self.output_path, '',
                                                               '',
                                                               {})
                ...
                entity_impacted_methods = self._get_field_invocation_in_methods_table(package_class, need_analyze_obj,
                                                                                      annotations, commit_or_branch,
                                                                                      class_id)
                ...
                entity_impacted_methods = self._get_method_invocation_in_methods_table(package_class, method_param,
                                                                                       commit_or_branch)
                method_db = self.sqlite.select_data(
                    f'SELECT annotations FROM methods WHERE method_id = {need_analyze_obj.get("method_id")}')[0]
                is_override_method = 'Override' in method_db['annotations']
                if is_override_method:

                    if class_entity['extends_class']:
                        abstract_package_class, method_params = self._is_method_param_in_extends_package_class(
                            method_param, class_entity['extends_class'], 'True', commit_or_branch)
                        if abstract_package_class:
                            extends_methods = self._get_method_invocation_in_methods_table(abstract_package_class,
                                                                                           method_params,
                                                                                           commit_or_branch)
                    ...
                    if class_entity['implements']:
                        ...
                        if class_implements_obj:
                            implements_package_class = class_implements_obj[0].get('package_name') + '.' + \
                                                       class_implements_obj[0].get('class_name')
                            implements_package_class, method_params = self._is_method_param_in_extends_package_class(
                                method_param, implements_package_class, 'False', commit_or_branch)
                            if implements_package_class:
                                implements_methods = self._get_method_invocation_in_methods_table(
                                    implements_package_class, method_params, commit_or_branch)
                    ...
                    class_method_db = self.sqlite.select_data(
                        f'SELECT method_id FROM methods WHERE class_id = {class_id} and method_name = "{method_name}"')
                    if not class_method_db:
                        extends_package_class, method_params = self._is_method_param_in_extends_package_class(
                            method_param, class_entity['extends_class'], 'False', commit_or_branch)
                        if extends_package_class:
                            extends_methods = self._get_method_invocation_in_methods_table(extends_package_class,
                                                                                           method_params,
                                                                                           commit_or_branch)
                            entity_impacted_methods += extends_methods
    ...
    def _is_method_param_in_extends_package_class(self, method_param, extends_package_class, is_abstract,
                                                  commit_or_branch):
        ...
        extends_class_db = self.sqlite.select_data(
            f'SELECT class_id, extends_class FROM class WHERE package_name = "{extends_package}" and class_name = "{extends_class_name}" and project_id = {self.project_id} and commit_or_branch = "{commit_or_branch}"')
        if not extends_class_db:
            extends_class_db = self.sqlite.select_data(
                f'SELECT class_id, extends_class FROM class WHERE package_name = "{extends_package}" and class_name = "{extends_class_name}" and project_id = {self.project_id}')
            if not extends_class_db:
                return None, None
        extends_class_id = extends_class_db[0]['class_id']
        methods_db_list = self.sqlite.select_data(
            f'SELECT * FROM methods WHERE class_id = {extends_class_id} and method_name = "{method_name}" and is_abstract = "{is_abstract}"')
        filter_methods = [method for method in methods_db_list if
                          len(json.loads(method.get('parameters', '[]'))) == len(method_arguments)]
        if not filter_methods:
            if extends_class_db[0]['extends_class']:
                return self._is_method_param_in_extends_package_class(method_param,
                                                                      extends_class_db[0]['extends_class'], is_abstract,
                                                                      commit_or_branch)
            else:
                return None, None
    ...
    def _get_extends_package_class(self, package_class):
        ...
        extends_package_class_db = self.sqlite.select_data(
            f'SELECT package_name, class_name FROM class WHERE project_id = {self.project_id} AND extends_class="{package_class}"')
        if extends_package_class_db:
            extends_package_class_list = [f'{class_item["package_name"]}.{class_item["class_name"]}' for class_item in
                                          extends_package_class_db]
    ...
    def _get_right_class_entity(self, class_db_list, commit_or_branch):
        right_class_entity = next((item for item in class_db_list if item.get("commit_or_branch") == commit_or_branch),
                                  None)
        if right_class_entity is None:
            right_class_entity = next(
                (item for item in class_db_list if item.get("commit_or_branch") == self.commit_or_branch_new), None)
        return right_class_entity

    # Step 5.2
    def _get_entity_invocation_in_methods_table(self, package_class: str):
        return self.sqlite.select_data(
            f'''SELECT method_id, class_id, method_name, parameters, return_type, is_api, api_path, documentation, body FROM methods WHERE project_id = {self.project_id} AND json_extract(method_invocation_map, '$."{package_class}".entity') IS NOT NULL''')

    # Step 5.2
    def _get_entity_invocation_in_field_table(self, package_class: str):
        return self.sqlite.select_data(
            f'''SELECT field_id, class_id, annotations, field_type, field_name, is_static, documentation FROM field WHERE project_id = {self.project_id} AND field_type = "{package_class}"''')
    ...
    def _get_field_invocation_in_methods_table(self, package_class, field_obj, annotations, commit_or_branch, class_id):
        ...
        if not is_static:
            ...
            sql = f'SELECT method_id, class_id, method_name, parameters, return_type, is_api, api_path, documentation, body FROM methods WHERE project_id = {self.project_id} AND (' + ' OR '.join(
                json_extract_sql_list) + ')'
        else:
        ...
        if not methods:
            field_method_name_list = ['get' + field_name_capitalize, 'set' + field_name_capitalize,
                                      'is' + field_name_capitalize]
            field_method_name_str = '"' + '","'.join(field_method_name_list) + '"'
            methods = self.sqlite.select_data(
                f'SELECT method_id, class_id, method_name, parameters, return_type, is_api, api_path, documentation, body FROM methods WHERE class_id = {class_id} AND method_name in ({field_method_name_str})')
    ...
    def _handle_impacted_fields(self, impacted_fields: list, source_node_id):
        for impacted_field in impacted_fields:
            ...
            class_entity = self.sqlite.select_data(
                f'SELECT package_name, class_name, commit_or_branch, filepath FROM class WHERE class_id={class_id}')[0]
            class_name = class_entity['class_name']
            package_name = class_entity['package_name']
            package_class = f'{package_name}.{class_name}'
            commit_or_branch = class_entity['commit_or_branch']
            class_filepath = class_entity['filepath']
            impacted_field_node_id = self.view.create_node_category(class_name, impacted_field['field_name'],
                                                                    constant.NODE_TYPE_FIELD,
                                                                    constant.DIFF_TYPE_IMPACTED, None, class_filepath,
                                                                    impacted_field['documentation'], '', {})
            ...
            self._add_to_need_analyze_obj_list('java', package_class, impacted_field['field_name'], None,
                                               commit_or_branch, extend_dict)
    ...
    def _add_to_need_analyze_obj_list(self, file_type: str, package_class: str, field_name: str or None,
                                      method_param: str or None, commit_or_branch: str, mapper_extend_dict: dict):
        ...
        is_exist = [obj for obj in self.need_analyze_obj_list if
                    self.check_dict_keys_equal_values(need_analyze_entity, obj)]
        if not is_exist:
    ...
    def _start_analysis_diff_and_impact(self):
        ...
        try:
            ...
            os.remove(os.path.join(self.temp_path, 'Occupy.ing'))
        finally:
            pass
    ...
    def analyze_two_branch(self, branch_first, branch_second, **kwargs):
        ...
        # self.file_path = os.path.join(self.project_dir, self.project_name)
        self.project_id = self.sqlite.add_project(self.project_name, self.git_url, self.branch_name, branch_first,
                                                  branch_second)
        # 已有分析结果
        self.result_html_file_name = os.path.join(self.output_path,
                                                  self.project_name,
                                                  f'{branch_second.replace("/", "#")}..{branch_first.replace("/", "#")}.cci.html')
        self._can_analyze(self.result_html_file_name)
        # 无此项目, 先clone项目
        project_dir = os.path.join(self.temp_path, self.project_name)
        if not os.path.exists(project_dir):
            logging.info(f'Cloning project: {self.git_url}')
            os.system(f'git clone -b {branch_first} {self.git_url} {project_dir}')

        dependents: list[dict] = kwargs.get('dependents', [])
        self._clone_dependents_project(dependents)

        self._occupy_project()
        self.diff_parse_map = self._get_branch_diff_parse_map(project_dir, branch_first, branch_second)
        self.xml_parse_results_new, self.xml_parse_results_old = self._parse_branch_project(project_dir,
    ...
    def analyze_two_commit(self, branch, commit_first, commit_second, **kwargs):
        ...
        self.project_name = self.git_url.split('/')[-1].split('.git')[0]
        self.output_path = os.path.join(self.temp_path, self.project_name)

        self.project_id = self.sqlite.add_project(self.project_name, self.git_url, self.branch_name,
                                                  self.commit_or_branch_new, self.commit_or_branch_old)
        # 已有分析结果
        self.result_html_file_name = os.path.join(self.output_path,
                                                  self.project_name,
                                                  f'{self.commit_or_branch_old}..{self.commit_or_branch_new}.cci.html')
        self._can_analyze(self.result_html_file_name)

        # 无此项目, 先clone项目
        if not os.path.exists(self.output_path):
            logging.info(f'Cloning project: {self.git_url}')
            os.system(f'git clone -b {self.branch_name} {self.git_url} {self.output_path}')

        dependents: list[dict] = kwargs.get('dependents', [])
        self._clone_dependents_project(dependents)

        self._occupy_project()
        self.diff_parse_map = self._get_diff_parse_map(self.output_path, self.branch_name, self.commit_or_branch_new,
                                                       self.commit_or_branch_old)

        self.xml_parse_results_new, self.xml_parse_results_old = self._parse_project(self.output_path,
    ...
    def analyze_class_method(self, branch, commit_id, package_class, method_nums, **kwargs):
        ...
        self.commit_or_branch_new = self.commit_or_branch_new[0: 7] if len(
            self.commit_or_branch_new) > 7 else self.commit_or_branch_new
        self.project_name = self.git_url.split('/')[-1].split('.git')[0]
        self.output_path = os.path.join(self.temp_path, self.project_name)

        self.project_id = self.sqlite.add_project(self.project_name, self.git_url, self.branch_name,
                                                  self.commit_or_branch_new, f'{package_class}.{method_nums}')
        class_name = package_class.split("/")[-1].replace('.java', '')
        cci_path = f'{branch.replace("/", "#")}_{commit_id}_{class_name}_{method_nums}.cci.html'
        self.result_html_file_name = os.path.join(self.output_path, self.project_name, cci_path)
        self._can_analyze(self.result_html_file_name)
        ...
        logging.info('Git pull project to HEAD')
        os.system(f'cd {self.output_path} && git checkout {branch} && git pull')
        time.sleep(1)

        if not method_nums:
            method_nums_all = []
            # todo
            class_db = self.sqlite.select_data('SELECT * FROM class WHERE project_id = ' + str(
                self.project_id) + ' and filepath LIKE "%' + package_class + '"')
            if not class_db:
        ...
        self.xml_parse_results_new, self.xml_parse_results_old = self._parse_project(self.output_path,
                                                                                     self.commit_or_branch_new, None)
